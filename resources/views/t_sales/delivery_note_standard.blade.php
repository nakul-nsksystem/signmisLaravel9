...
@section('content')
	@forelse ($chunks as $items)
		...
		{{-- [控え] ヘッダー・空行・フッター --}}
		@include('t_sales.delivery_note_standard_header', [$header, 'isFirst'=>TRUE, ++$page, $total_page, 'title'=>'納 品 書 [控]'])
		@foreach ($items as $item)
			@include('t_sales.delivery_note_standard_detail', [$item])
		@endforeach
		@if ($loop->last)
			@include('layouts.app_report_empty_row', ['rows'=>$empty_row_count, 'cols'=>8])
		@endif
		@include('t_sales.delivery_note_standard_footer', ['isFirst'=>TRUE, 'isSecond'=>FALSE, 'isLast'=>$loop->last])

		{{-- [納品書] ヘッダー・空行・フッター --}}
		@include('t_sales.delivery_note_standard_header', [$header, 'isFirst'=>FALSE, $page, $total_page, 'title'=>'納 品 書'])
		@foreach ($items as $item)
			@include('t_sales.delivery_note_standard_detail', [$item])
		@endforeach
		@if ($loop->last)
			@include('layouts.app_report_empty_row', ['rows'=>$empty_row_count, 'cols'=>8])
		@endif
		@include('t_sales.delivery_note_standard_footer', ['isFirst'=>TRUE, 'isSecond'=>TRUE, 'isLast'=>$loop->last])

		{{-- [受領書] ヘッダー・空行・フッター --}}
		@include('t_sales.delivery_note_standard_header', [$header, 'isFirst'=>FALSE, $page, $total_page, 'title'=>'受 領 書'])
		@foreach ($items as $item)
			@include('t_sales.delivery_note_standard_detail', [$item])
		@endforeach
		@if ($loop->last)
			@include('layouts.app_report_empty_row', ['rows'=>$empty_row_count, 'cols'=>8])
		@endif
		@include('t_sales.delivery_note_standard_footer', ['isFirst'=>FALSE, 'isSecond'=>FALSE, 'isLast'=>$loop->last])
	@empty
	@endforelse
@endsection
