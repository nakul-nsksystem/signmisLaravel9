...
export class PdtGrouperDeliveryAt extends PdtGrouperAbstract { 
    /**
     * 定義取得
     */
    public get def():ITProductionGroupDef {
        return PdtGrouperDeliveryAt.getDef() ; 
    } 
    ...
    {
        // @ts-ignore
        const keys = list.map(x => x.t_project_product.calced_delivery_date) ;     
        return keys ; 
    }
    ...
    public getKey(process:TProjectProductProcess):string{
        // @ts-ignore
        return process.t_project_product.calced_delivery_date ; 
    }
    ...
    protected filterByKey(key:string , list:Array<TProjectProductProcess>):Array<TProjectProductProcess>{
        // @ts-ignore
        const filterd = list.filter(x => x.t_project_product.calced_delivery_date === key ) ; 
        return filterd ; 
    }
     ...
     */
     public getConvertedName(key:string):string{
        // @ts-ignore
        return _.isNil( key) ? "-" : DayJsEx.formatDate(key)   ; 
    }
