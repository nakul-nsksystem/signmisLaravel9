                        ...
                        <div class="row mx-1">
                            <div class="col-12 pl-xl-0">
                                <div class="form-group">
                                    ...
                                    <validation-provider name="仕入先" rules="required" immediate v-slot="{ errors }">
                                ...
                                <table class="table table-striped table-dark table-hover list">
                                    <thead>
                                        <tr class="text-nowrap">
                                            ...
                                            <th scope="col" style="display:none">仕入締id</th>
                                            <th scope="col">締処理日</th>
                                            <th scope="col" class="d-none d-md-table-cell">仕入合計</th>
                                            <th scope="col" class="d-none d-md-table-cell">検収額</th>
                                            <th scope="col">前回締処理日</th>
                                            <th scope="col" style="display:none">仕入先id</th>
                                            <th scope="col" >仕入先</th>
                                            <th scope="col" >検収書郵送区分</th>
                                        </tr>
                                    </thead>
...
export default {
    ...
    data : function () {
        return {
            ...
            dSelectedInvoiceList : [] ,             // 検収書の選択データ
            ...
            dCanBeInputCustomer : false ,           // 仕入先の入力可否判定用(true:入力可能 false:入力不可)
            dLoadingCheck : false ,                 // 非同期apiでの読込中フラグ(締確認)
            dLoadingComplete : false ,              // 非同期apiでの読込中フラグ(締処理)
            dLoadingInvoice : false ,               // 非同期apiでの読込中フラグ(検収書)
    ...
    computed : {
        ...
        cIsRequired : function () {
            // 仕入先<checkbox>チェックが付いている & <m-customer-ref-input>未入力時は必須とする
        ...
        cMBranchId : {
            get : function () {
                return this.dSearch.m_branch_id ;
            } ,
            set : function (value) {
                this.dSearch.m_branch_id = value ;
                // 拠点の締日
                this.dSearch.closing_date = this.getBranchClosingDate(value) ;
            } ,
        } ,
        cRowsList : function() {
    ...
    methods : {
        ...
        outputInvoice : async function () {
            if (!confirm('検収書を発行してもよろしいですか?')) return ;
            ...
            link.setAttribute('download', '検収書.zip') ;
            link.click();
            URL.revokeObjectURL(link.href) ;

            this.dInfoMessage = "検収書が発行されました" ;
        ...
        getBranchClosingDate : function (branchId)
        {
            return this.masterStore.MBranches.find(x => x.id == branchId)?.closing_date ;
        } ,
    } ,
    created : function () {
        if (this.mainStore.isAppReady) {
            ...
            // 拠点の締日
            this.dSearch.closing_date = this.getBranchClosingDate(this.mainStore.loginMUser.m_branch_id) ;
...
<style scoped>
    /* 結果行(複数の仕入先名を1行で表示してるので長い名称時に省略記号を表記) */
    td {
