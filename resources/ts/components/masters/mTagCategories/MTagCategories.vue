        ...
        <table class="table table-striped table-dark">
            <thead>
                <tr class="text-nowrap" @click.prevent="sort">
                    ...
                    <th scope="col" id="tag_category_key"   :class="setStyleSort('tag_category_key')">カテゴリキー</th>
                    <th scope="col" id="tag_category_name"  :class="setStyleSort('tag_category_name')">名称</th>
                    <th scope="col" id="tag_short_name"     :class="setStyleSort('tag_short_name')">略称</th>
            ...
            <tbody>
                <tr v-for="n in cList" :key="n.id">
                    ...
                    <td>{{n.id}}</td>
                    <td>{{n.tag_category_key}}</td>
                    <td>{{n.tag_category_name}}</td>
                    <td>{{n.tag_short_name}}</td>
    ...
    methods : {
        destroy : async function(id){
            ...
            try {
                ...
                const deleted  = this.mainStore.masters.MTagCategories.find(e => e.id === id) ;
                const delIndex = this.mainStore.masters.MTagCategories.indexOf(deleted) ;
                if (delIndex != -1) this.mainStore.masters.MTagCategories.splice(delIndex, 1);
            } catch (error)  {
                // handle error
