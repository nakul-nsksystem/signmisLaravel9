<template>
    <validation-observer v-slot="{ invalid }">
        <div class="row" v-if="cIsShow">
            <div class="col">
                    ...
                    <div class="row">
                        ...
                        <contents-header-right>
                            <div class="float-right pt-2">
                                ...
                                <button type="submit" class="btn btn-primary" @click.prevent="printDialogOpen()" :disabled="cIsDisabledPrintButton">納品書</button>
                                <button type="submit" class="btn btn-secondary" @click.prevent="printDialogOpenView()" :disabled="cIsDisabledPrintButton" v-if="$$cIsNskDev">確認用</button>
                ...
                <div class="border-top">
                    ...
                    <div class="row mx-1">
                        <div class="col-12 col-lg-2 pl-lg-0">
                            <validation-provider name="売上日" :rules="`required|custom_rule_date_range:${m$cSmEditableTerm}|custom_rule_date_or_later:my_custom_rule_day_min`" immediate v-slot="{ errors }">
                                <div class="form-group">
                                    <label>売上日</label>
                                    <ex-v-date-picker
                        ...
                        <div class="col-12 col-lg-3 pl-lg-0">
                            <validation-provider name="客先担当者" rules="custom_rule_max_bytelength:15,30" immediate v-slot="{ errors }">
                                <div class="form-group">
                                    <label for="contact">客先担当者</label>
                                    <input class="form-control" id="contact" v-model="dRow.contact">
                                    <span class="validation-error">{{ errors[0] }}</span>
                                </div>
                            </validation-provider>
                        </div>
                    ...
                    <div class="row mx-1">
                        <div class="col-12 col-lg-2 pl-lg-0">
                            <validation-provider name="納品書" rules="required|excluded:0" v-slot="{ errors }">
                                <div class="form-group">
                                    <label>納品書</label>
                                    <m-kv-select
                    ...
                    <validation-observer v-slot="{ invalid }">
                        ...
                        <div class="row mx-1">
                            <div class="col-12 col-lg-1 pl-lg-0">
                                <validation-provider name="売上" rules="required|excluded:0" v-slot="{ errors }">
                                    <div class="form-group">
                                        <label>売上</label>
                                        <m-kv-select
                                            v-model="dSelectedRows.ship_m_kv_id"
                                            :selected-item.sync="dSelectedShipMkv"
                                            :kv-key="'sales_management-ship'"
                                        />
                                        <span class="validation-error">{{ errors[0] }}</span>
                            ...
                            <div class="col-12 col-lg-4 pl-lg-0">
                                <validation-provider name="物件名・品名" rules="required|custom_rule_max_bytelength:23,46" immediate v-slot="{ errors }">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input class="form-control" id="product_name" v-model="dSelectedRows.product_name">
                                        </div>
                                        <span class="validation-error">{{ errors[0] }}</span>
                                    </div>
                                </validation-provider>
                            ...
                            <div class="col-12 col-lg-2 pl-lg-0">
                                <div class="form-group">
                                    <label for="customer_order_no">客先注文No.</label>
                                    <input class="form-control" id="customer_order_no" v-model="dSelectedRows.customer_order_no">
                                </div>
                            </div>
                            <div class="col-12 col-lg-5 pl-lg-0">
                                <div class="form-group">
                        ...
                        <div class="row mx-1">
                            ...
                            <div class="col-12 col-lg-1 pl-lg-0">
                                <validation-provider name="単位" rules="required|excluded:0" v-slot="{ errors }">
                                    <div class="form-group">
                                        <label>単位</label>
                                        <m-kv-select
                            ...
                            <div class="col-12 col-lg-5 pl-lg-0">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="slip_memo" v-model="dSelectedRows.slip_memo">
                                </div>
                            </div>
                        </div>
        ...
        <t-sale-project-list
            v-show="dIsModalOpen_TProject"
            :is-open.sync="dIsModalOpen_TProject"
            :p-branch-id.sync="dRow.m_branch_id"
            :p-customer-id.sync="dRow.m_customer_id"
            :p-customer-name.sync="dRow.m_customer_name"
            :p-shipped-at.sync="dRow.accounting_date"
...
export default {
    ...
    data : function () {
        return {
            dApiName : "tSale",
            dApiDeliveryName : "tSale/deliveryNote",
            dOriginal : undefined ,                             // オリジナルデータ(更新確認メッセージで使用)
            dRow : {
                id : 0,                                         // 新規:0 新規登録後は1以降になる
                shipped_at : null,                              // 出荷日
                accounting_date : null,                         // 計上日(売上日)
                m_branch_id : 0,                                // 拠点
                m_user_id : 0,                                  // 売上担当者
                m_customer_id : 0,                              // 取引先
                m_customer_name : "",                           // 取引先名
                delivery_destination_m_customer_id : 0,         // 納品先
                slip_m_kv_id : 1160001,                         // 伝票区分[掛売]
                account_m_kv_id : 1060000,                      // 締区分[未請求]
                delivery_note_m_kv_id : 1080001,                // 納品書区分[本伝]
                designated_delivery_note_m_kv_id : 1090000,     // 指定納品書区分[無し]
                shipping_label_m_kv_id : 1100000,               // 送り状区分[無し]
            ...
            dLastCompleteDay : null ,                           // チェック用の締情報(取引先の最終締処理日)
            dCustomerSelectedItem : null,                       // 取引先データ
            dIsValid_DetailAddButton : false,                   // 明細追加ボタンの有効状態(false:無効 true:有効)
            dIsShow_DetailArea : false,                         // 明細入力エリアの表示状態(false:表示しない true:表示する)
            dSelectedRows : {},                                 // 選択行
            dSelectedShipMkv : {} ,                             // 選択されている売上区分
            dAddRowNo : -10000,                                 // 新規作成時の仮行番号は加算(-9999, -9998, -9997, ...)で管理
            dIsModalOpen_TProject : false,                      // モーダル画面(発注)を起動するフラグ Modal画面+vuejsの条件付きレンダリング制御
    ...
    computed : {
        ...
        cEndpointDelivery : function () {
            // 納品書のURL
            return `api/${this.dApiDeliveryName}/${this.dRow.id}` ;
        } ,
        cEndpointDeliveryViewMode : function () {
            // 納品書のURL(view表示用)
            return `api/${this.dApiDeliveryName}/${this.dRow.id}/view` ;
        } ,
        ...
        cIsDisabledSaveButton : function () {
            // 明細追加ボタンの有効状態(請求済み Or 明細入力エリアが表示されている場合 Or 明細が1件も登録されてない場合は無効)
        ...
        cIsDisabledPrintButton : function () {
            // 新規登録前・納品書区分[無し]の場合に印刷ボタンは無効
            return this.dRow.id == 0 || this.dRow.delivery_note_m_kv_id == 1080000 ;
        } ,
        ...
        cDetails : function () {
            ...
            let result = this.dRow.t_sale_details ;
        ...
        cIsRequiredQty : function () {
            // 出荷区分が記入用[1200005]以外の場合に数量0はエラーとする
            return (this.dSelectedRows.ship_m_kv_id != 1200005 &&  this.dSelectedRows.qty == 0) ? 0 : 1 ;
        } ,
        cIsRequiredTotalPrice : function () {
            // 出荷区分が記入用[1200005]の場合に金額0以外はエラーとする
            return (this.dSelectedRows.ship_m_kv_id == 1200005 &&  this.dSelectedRows.total_price != 0) ? 1 : 0 ;
        } ,
        cCustomerItem : {
            get : function () {
            ...
            set : function (value) {
                ...
                if (value !== undefined && this.dCustomerSelectedItem?.id != value.id) {
                    ...
                    // 新規の場合に納品先はデフォルトで取引先をセット
                    if (this.dRow.id == 0 && value.id) {
                        this.dRow.delivery_destination_m_customer_id = value.id ;
                    }

                    // 新規の場合に取引先の納品書をセット
                    if (this.dRow.id == 0 && value.delivery_note_m_kv_id) {
                        this.dRow.delivery_note_m_kv_id = value.delivery_note_m_kv_id ;
                    }
        ...
        cValidationCheck : function () {
            ...
            for (const detail of this.dRow.t_sale_details) {
                i++ ;
                // 削除フラグは無視
                if (detail.deleted_at == null) {
                    // 物件名・品名の桁数確認
                    const len = StringUtil.getByteLengthOfShiftJIS(detail.product_name) ;
                    if (len > 46) {
                        invalidMessage += (", " + i) ;
                    }
                }
            ...
            if (invalidMessage) {
                ...
                invalidMessage = invalidMessage.substr(2) + "行目：[物件名・品名]の桁数を確認してください" ;
            }

            return invalidMessage ;
        } ,
        cReInvoiceInfoMessage : function () {
            return (this.dSelectedShipMkv?.re_invoice_flg) ? "返還として処理します" : "" ;
        } ,
    ...
    methods : {
        ...
        postData : async function () {
            ...
            try {
                this.dRow.shipped_at                        = this.dRow.accounting_date ;
                this.dRow.accounting_m_customer_id          = this.dRow.m_customer_id ;
                // 最終更新者・日付
        ...
        taxCalcDetail : async function () {
            ...
            // 各明細の金額・消費税を計算
            const details = this.dRow.t_sale_details.map(function(item) {
                // 税率を算出
        ...
        taxCalcReceipt : async function () {
            ...
            // 明細の金額を集計
            let      total_price_normal        = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170001 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 0 ? item.total_price : 0) ; }, 0) ; /*      金額 通常   */
            let      total_price_reduced       = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170002 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 0 ? item.total_price : 0) ; }, 0) ; /*      金額 軽減   */
            let      total_price_tax_exemption = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170000 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 0 ? item.total_price : 0) ; }, 0) ; /*      金額 非課税 */
            let   re_total_price_normal        = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170001 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 1 ? item.total_price : 0) ; }, 0) ; /* 軽減 金額 通常   */
            let   re_total_price_reduced       = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170002 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 1 ? item.total_price : 0) ; }, 0) ; /* 軽減 金額 軽減   */
            let   re_total_price_tax_exemption = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170000 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 1 ? item.total_price : 0) ; }, 0) ; /* 軽減 金額 非課税 */
            // 明細の消費税を集計
            const    tax_normal                = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170001 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 0 ? item.tax         : 0) ; }, 0) ; /*      消費税 通常 */
            const    tax_reduced               = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170002 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 0 ? item.tax         : 0) ; }, 0) ; /*      消費税 軽減 */
            const re_tax_normal                = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170001 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 1 ? item.tax         : 0) ; }, 0) ; /* 軽減 消費税 通常 */
            const re_tax_reduced               = this.dRow.t_sale_details.reduce(function(sum, item) { return sum + (item.tax_m_kv_id == 1170002 && item.deleted_at == null && item.ship_m_kv.re_invoice_flg == 1 ? item.tax         : 0) ; }, 0) ; /* 軽減 消費税 軽減 */
        ...
        ifNewSetIdToNull : async function () {
            this.dRow.t_sale_details.forEach((item) => {
                // 新規作成の明細idはnullでapiに渡す
        ...
        printDialogOpen : function () {
            // 納品書
            window.open(this.cEndpointDelivery, '_blank') ;
        } ,
        printDialogOpenView : function () {
            // 納品書(vie表示用：開発テスト)
            window.open(this.cEndpointDeliveryViewMode, '_blank') ;
        } ,
        initRow : function () {
            // 明細の初期化
            this.dSelectedRows = {
        ...
        selectRow : function (row) {
            ...
            const index = this.dRow.t_sale_details.findIndex(e => e.id === row.id) ;
            if (index != -1 && !this.dRow.t_sale_details[index].deleted_at) {
        ...
        updateRow : async function () {
            ...
            // 選択されている売上区分を設定
            detail.ship_m_kv         = this.dSelectedShipMkv ;
            ...
            if (detail.id == 0) {
                ...
                this.dRow.t_sale_details.push(detail) ;
            } else {
                // 明細更新 & 明細追加した後に画面内で更に更新
                const index = this.dRow.t_sale_details.findIndex(e => e.id === detail.id) ;
                if (index != -1) {
                    this.dRow.t_sale_details.splice(index, 1, detail) ;
                }
            }
        ...
        deleteRowDetail : function (row) {
            ...
            if (index != -1) {
                ...
                if (detail.t_sale_id == 0) {
                    // 新規に追加した行は物理削除
                    this.dRow.t_sale_details.splice(index, 1) ;
                } else {
                    // 登録済みの明細行
                    this.dRow.t_sale_details.splice(index, 1, detail) ;
                }
        ...
        modalShow : function () {
            // 取引先名を呼び出し側に渡す必要がるためこの段階でセットする
            this.dRow.m_customer_name = this.$refs.customerInput?.name ?? "" ;

            // 物件検索(モーダル)を表示
            this.dIsModalOpen_TProject = true ;
        } ,
        modalShow2 : function () {
            // 材料検索(モーダル)を表示
        ...
        modalHide : function (args) {
            ...
            for (const n of args) {
                // 売上用に抽出
                const sale = {
                    id                         : ++this.dAddRowNo                            , // 売上明細id
                    t_sale_id                  : 0                                           , // 売上id
                    t_project_id               : n.t_project_id                              , // 物件id
                    t_project                  : { cd : n.t_project_cd }                     , // 物件cd
                    t_project_product_id       : n.product_id                                , // 物件商品id
                    product_name               : n.product_name                              , // 品名(物件名)
                    customer_order_no          : n.customer_order_no                         , // 客先注文No.
                    memo                       : n.memo                                      , // 備考
                    unit_m_kv_id               : n.unit_m_kv_id                              , // 単位区分
                    qty                        : this.cNu2zr(n.qty)                          , // 数量
                    price                      : this.cNu2zr(n.price)                        , // 単価
                    total_price                : this.cNu2zr(n.total_price)                  , // 金額
                    product_size_x             : this.cNu2zr(n.product_size_x)               , // 製品サイズX
                    product_size_y             : this.cNu2zr(n.product_size_y)               , // 製品サイズY
                    tax_rate                   : 10                                          , // 税率[10%]
                    tax_m_kv_id                : 1170001                                     , // 税区分[外税10%]
                    ship_m_kv_id               : n.ship_m_kv_id                              , // 出荷区分[]
                    ship_m_kv                  : { id : n.ship_m_kv_id, re_invoice_flg : 0 } , // 出荷区分[] 初期追加用
                    complete_m_kv_id           : 1210001                                     , // 完了区分[完了]
                    delivery_note_disp_m_kv_id : 1240001                                     , // 納品書表示区分[表示]
                    classification_m_kv_id     : 1270000                                     , // 分類区分[未設定]
                ...
                if (this.dRow.id == 0 && !this.dRow.m_customer_id && this.dRow.m_branch_id == n.m_branch_id) { this.dRow.m_customer_id = n.m_customer_id ; }

                // 最終着日を上書き
                if (n.shipped_at) { this.dRow.accounting_date = n.shipped_at ; }

                // 客先担当者：設定されてなければ上書き
                if (!this.dRow.contact && n.contact) { this.dRow.contact = n.contact ; }

                // 適応：客先担当者+客先注文No. 設定されてなければ上書き
                if (!this.dRow.slip_memo && (n.contact || n.customer_order_no)) {
                    this.dRow.slip_memo = [n.contact, n.customer_order_no].join(" ").trim() ;
                }

                // データ追加
                this.dRow.t_sale_details.push(sale) ;
            }
        } ,
        modalHide2 : function (args) {
            ...
            for (const n of args) {
                ...
                const material = {
                    ...
                    product_name               : n.material_name                    , // 品名・銘柄
                    qty                        : n.qty                              , // 数量
                    price                      : 0                                  , // 単価
                    total_price                : 0                                  , // 金額
                    product_size_x             : n.material_size_x                  , // 製品サイズX
                    product_size_y             : n.material_size_y                  , // 製品サイズY
                    unit_m_kv_id               : n.unit_m_kv_id                     , // 単位区分
        ...
        moveAt : function(index) {
            if (index === this.dDragIndex || index > this.dRow.t_sale_details.length -1 || this.dDragIndex > this.dRow.t_sale_details.length - 1) {
                return ;
            }

            // 配列の要素を指定位置に移動させる
            const value = this.dRow.t_sale_details[this.dDragIndex] ;
            this.dRow.t_sale_details.splice(this.dDragIndex, 1) ;
            this.dRow.t_sale_details.splice(index, 0, value) ;
        } ,
        ...
        setSequenceNumbering : function () {
            // order_noを連番に振り直し
            this.dRow.t_sale_details.map((value, no) => {
                value.order_no = no + 1 ;
                return value
            }) ;
        } ,
        ...
        initAllData : function ( isKeepCustomer = false ){
            ...
            if (!isKeepCustomer) this.dRow.m_customer_id =  0
            if (!isKeepCustomer) this.dRow.m_customer_name = ""
            if (!isKeepCustomer) this.dRow.delivery_destination_m_customer_id = 0
            if (!isKeepCustomer) this.dRow.account_m_kv_id = 1060000
            this.dRow.slip_m_kv_id = 1160001
            this.dRow.delivery_note_m_kv_id = 1080001
            this.dRow.designated_delivery_note_m_kv_id = 1090000
            this.dRow.shipping_label_m_kv_id = 1100000
            this.dRow.t_complete_id = 0
            ...
            this.dRow.t_sale_details.splice(0) ;
            ...
            this.dIsShow_DetailArea = false,
            this.dSelectedRows = {} ;
            this.dSelectedShipMkv = {} ;
            this.dAddRowNo = -10000 ;
            this.dIsModalOpen_TProject = false ;
        ...
        onNewButton : function() {
            ...
            const is = confirm("取引先・納品先を引き継ぎますか？") ;
            this.initAllData(is) ;

            if (this.$route.path !== "/t_sale/add") {
    ...
    created : function () {
        if (this.cIsNew) {
            ...
            this.dRow.m_user_id   = this.mainStore.loginMUser.id ;          // 売上担当者
        } else {
            // 変更
            this.getData() ;
        }
...
<style scoped>
    /* 明細：右揃え */
    th:nth-child(n+5):nth-child(-n+8),
    td:nth-child(n+5):nth-child(-n+8) {
        text-align: right;
    }
</style>
