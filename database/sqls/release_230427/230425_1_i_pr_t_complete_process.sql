             ...
             SELECT   pMBranchId, m_customer_id,            m_customer_id,  pMUserId,      1160003,    pCompleteDay, pCompleteDay, tax, tax_normal, tax_reduced,          pMUserId,          pMUserId  
               FROM (/* 金額集計に対して消費税の集計と端数処理 */                                                                                                     
                     SELECT m_customer_id,                                                                                                                            
                            SUM(IF(tax_m_kv_id IN(1170001, 1170002), ROUNDING(total_price * tax_rate, tax_fraction_m_kv_id), 0)) AS tax         , /* 税合計        */ 
                            SUM(IF(tax_m_kv_id  = 1170001          , ROUNDING(total_price * tax_rate, tax_fraction_m_kv_id), 0)) AS tax_normal  , /* 通常税率(10%) */ 
                            SUM(IF(tax_m_kv_id  =          1170002 , ROUNDING(total_price * tax_rate, tax_fraction_m_kv_id), 0)) AS tax_reduced   /* 軽減税率( 8%) */ 
                       FROM (/* 新旧消費税区分毎に金額集計 */                                               
