            ...
            -- CREATE OR REPLACE VIEW v_ledger_sales AS
        ...
        $details = DB::select(<<<__SQL_EOT__
            ...
            q_max_id AS (/* 伝票時に作成された消費税の判別用として抽出 */
                SELECT MAX(d.id) AS id /* Max明細id */
                 ...
                 INNER JOIN t_sale_details AS d /* 売上明細 */ ON h.id = d.t_sale_id
                 WHERE h.deleted_at           IS NULL                                    /* 論理削除 */
                   AND d.deleted_at           IS NULL                                    /* 論理削除 */
                   AND h.m_customer_id         = :m_customer_id1                         /* 取引先   */
            ...
            q_tax AS (/* 請求締時に作成した消費税(10%・8%)を別の行で出力したかったのでSubQueryにしてます */
                ...
                SELECT h.id                                    AS slip_no         , /* 伝票no   */
                       1                                       AS id              , /* id       */
                       h.accounting_date                       AS accounting_date , /* 売上日   */
                 ...
                 WHERE h.deleted_at           IS NULL                                    /* 論理削除       */
                   AND h.m_customer_id         = :m_customer_id2                         /* 取引先         */
                ...
                SELECT h.id                                    AS slip_no         , /* 伝票no   */
                  ...
                  LEFT JOIN m_kvs AS k1 /* 区分 */ ON h.slip_m_kv_id  = k1.id 
                 WHERE h.deleted_at           IS NULL                                    /* 論理削除       */
                   AND h.m_customer_id         = :m_customer_id3                         /* 取引先         */
                   AND h.accounting_date BETWEEN :trading_day_from3 AND :trading_day_to3 /* 売上日         */
            ...
            q_sales AS (
                SELECT h.id                                                                             AS slip_no         , /* 伝票no   */
                       ...
                       CASE WHEN h.slip_m_kv_id  = 1160003 THEN k3.kv_name ELSE d.product_name END      AS product_name    , /* 商品名   */
                       CASE WHEN d.ship_m_kv_id <> 1200005 THEN d.qty                          END      AS qty             , /* 数量     */
                       k1.kv_name                                                                       AS unit_m_kv_name  , /* 単位     */
                       CASE WHEN d.ship_m_kv_id <> 1200005 THEN d.total_price                  END      AS total_price     , /* 売上     */
                       CASE /* 税区分で対象となる消費税を出力 */
                            WHEN cu.account_m_kv_id = 1060001                              THEN d.tax /* 明細 */
                        --  WHEN cu.account_m_kv_id = 1060003 AND h.slip_m_kv_id = 1160003 THEN h.tax /* 締   */ /* 請求消費税は[q_tax]で実装 */
                            WHEN cu.account_m_kv_id = 1060002 AND q1.id IS NOT NULL        THEN h.tax /* 伝票 */
                            ELSE NULL
                       END                                                                              AS tax             , /* 消費税   */
                       d.slip_memo                                                                      AS d_slip_memo       /* 備考     */

                  FROM t_sales             AS h  /* 売上                */
                  LEFT JOIN t_sale_details AS d  /* 売上明細            */ ON h.id            = d.t_sale_id
                 INNER JOIN m_customers    AS cu /* 取引先              */ ON h.m_customer_id = cu.id
                  LEFT JOIN m_kvs          AS k1 /* 区分(単位)          */ ON d.unit_m_kv_id  = k1.id
                  LEFT JOIN m_kvs          AS k2 /* 区分(出荷)          */ ON d.ship_m_kv_id  = k2.id
                 ...
                 WHERE h.deleted_at           IS NULL                                    /* 論理削除         */
                   ...
                   AND h.m_customer_id         = :m_customer_id4                         /* 取引先           */
                   AND h.accounting_date BETWEEN :trading_day_from4 AND :trading_day_to4 /* 売上日           */
            ) ,
            ...
            q_payments AS (
                SELECT p.id                     AS id            , /* id       */
                       ...
                       p.payment_price          AS payment_price , /* 入金     */
                       '入金'                   AS category_name , /* カテゴリ */
                       CONCAT(TRIM(CONCAT('[入金：', IFNULL(k1.kv_name, ''), ' ', IFNULL(bill_no, ''), ' ', IFNULL(DATE_FORMAT(bill_due_day, '%Y/%m/%d'), ''))), ']')
                                                AS payment_name    /* 入金内容 */
                 ...
                 WHERE p.deleted_at           IS NULL                                    /* 論理削除 */
                   ...
                   AND p.m_customer_id         = :m_customer_id5                         /* 取引先   */
                   AND p.payment_day     BETWEEN :trading_day_from5 AND :trading_day_to5 /* 入金日   */
            ) ,
            ...
            q_first_balances AS(
                ...
                SELECT c.m_customer_id AS m_customer_id , /* 取引先id */
                 ...
                 WHERE c.dealing_m_kv_id       = 1010001                                 /* 売           */
                   AND c.id                    > 0                                       /* 通常請求のみ */
                   AND c.m_customer_id         = :m_customer_id6                         /* 取引先       */
            ...
            q_before_complets AS(
                SELECT c.m_customer_id AS m_customer_id ,      /* 取引先id     */
                 ...
                 WHERE c.dealing_m_kv_id  = 1010001            /* 売           */
                   AND c.id               > 0                  /* 通常請求のみ */
                   AND c.m_customer_id    = :m_customer_id7    /* 取引先       */
            ...
            q_after_complets AS(
                SELECT c.m_customer_id AS m_customer_id ,      /* 取引先id     */
                 ...
                 WHERE c.dealing_m_kv_id  = 1010001            /* 売           */
                   AND c.id              >  0                  /* 通常請求のみ */
                   AND c.m_customer_id    = :m_customer_id8    /* 取引先       */
            ...
            q_balances AS (
                SELECT SUM(balance) AS balance /* 残高 */
                  FROM (/* 初回残高 指定開始日以前で1件も無い場合に残高0として抽出 */
                        ...
                        /* 開始日より前の入金(締済分) */
                         UNION ALL
                        SELECT p.m_customer_id           AS m_customer_id , /* 取引先id */
                         ...
                         WHERE p.deleted_at      IS NULL                /* 論理削除 */
                           AND p.dealing_m_kv_id  = 1010001             /* 売       */
                           AND p.payment_day      < :trading_day_from9  /* 入金日   */
                        ...
                        SELECT p.m_customer_id           AS m_customer_id , /* 取引先id */
                         ...
                         WHERE p.deleted_at      IS NULL                /* 論理削除 */
                           ...
                           AND p.m_customer_id    = :m_customer_id9     /* 取引先   */
                           AND p.payment_day      < :trading_day_from10 /* 入金日   */
                         GROUP BY p.m_customer_id

                        /* 開始日より前の売上(締済分) */
                         UNION ALL
                        SELECT h.m_customer_id                                                 AS m_customer_id , /* 取引先id */
                         ...
                         INNER JOIN m_customers      AS cu /* 取引先   */ ON h.m_customer_id = cu.id
                         WHERE h.deleted_at      IS NULL                /* 論理削除 */
                           AND h.accounting_date  < :trading_day_from11 /* 売上日   */
                         GROUP BY h.m_customer_id
                        ...
                        SELECT h.m_customer_id                                                 AS m_customer_id , /* 取引先id */
                         ...
                         WHERE h.deleted_at      IS NULL                /* 論理削除 */
                           ...
                           AND h.m_customer_id    = :m_customer_id10    /* 取引先   */
                           AND h.accounting_date  < :trading_day_from12 /* 売上日   */
                         GROUP BY h.m_customer_id
                ...
                SELECT trading_day                                    AS trading_day     , /* 取引日   */
                  ...
                  FROM (/* 残高 */
                        SELECT NULL            AS trading_day     , /* 取引日   */
                               ...
                               NULL            AS total_price     , /* 売上     */
                               NULL            AS tax             , /* 消費税   */
                               NULL            AS payment_price   , /* 入金     */
                        ...
                        SELECT accounting_date AS trading_day     , /* 取引日   */
                               ...
                               NULL            AS payment_price   , /* 入金     */
                               NULL            AS balance         , /* 残高     */
                               total_price     AS carried_forward   /* 繰越     */
                          FROM q_sales
                        ...
                        SELECT payment_day        AS trading_day     , /* 取引日   */
                               ...
                               id                 AS id              , /* id       */
                               category_name      AS category_name   , /* カテゴリ */
                               payment_name       AS product_name    , /* 商品名   */
                               NULL               AS project_cd      , /* 物件cd   */
                               NULL               AS qty             , /* 数量     */
                               NULL               AS unit_m_kv_name  , /* 単位     */
                               NULL               AS total_price     , /* 売上     */
            ...
            SELECT DATE_FORMAT(trading_day, '%Y-%m-%d') AS trading_day     , /* 取引日   */
                   ...
                   unit_m_kv_name                       AS unit_m_kv_name  , /* 単位     */
                   CAST(total_price     AS SIGNED)      AS total_price     , /* 売上額   */
                   CAST(tax             AS SIGNED)      AS tax             , /* 消費税   */
                   CAST(payment_price   AS SIGNED)      AS payment_price   , /* 入金額   */
