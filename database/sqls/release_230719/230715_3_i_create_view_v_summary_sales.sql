...
q_complete_details AS (
    SELECT target_t_complete_id                                                    AS target_t_complete_id , /* 対象請求id */
           SUM(CASE WHEN payment_m_kv_id <> 1150008 THEN payment_price ELSE 0 END) AS payment_price        , /* 入金       */
...
SELECT br.id                                        AS m_branch_id     , /* 拠点id     */ 
       ...
       SUM(c.total_price        )                   AS total_price     , /* 売上       */ 
 ...
 ORDER BY m_branch_cd, due_date, m_user_cd, m_customer_cd
;
