...
CREATE 
PROCEDURE pr_t_complete_validation(
...
BEGIN
    ...
    IF NOT pIsComplete THEN
        ...
        IF cntComplete IS NOT NULL THEN
            -- 締処理日以降にデータがある場合はNG
            SET pMessage = CONCAT('処理日以降に締データが存在します[', IF(pDealingMkvId = 1010001, '売', '買'), ']', cntComplete) ;
        ...
        ELSE
            ...
            IF cnt = 0 THEN
                -- 解除するデータがない場合はNG
                SET pMessage = CONCAT('対象データが存在しません[', IF(pDealingMkvId = 1010001, '売', '買'), '][締対象日：', pCompleteDay, ']') ;
            END IF ;
        END IF ;
    ...
    ELSE
        SET pMessage = CONCAT('締', IF(pIsComplete, '', '解除'), '確認を行いました [', IF(pDealingMkvId = 1010001, '売', '買'), '][締対象日：', pCompleteDay, '][件数：', cnt, ']') ;
