...
q_unclaimed AS (
    ...
    SELECT br.id                                                                                                          AS m_branch_id             , /* 拠点id       */
           ...
           /* 最終請求日からの経過月を表示 */
           PERIOD_DIFF(DATE_FORMAT(CURRENT_DATE, '%Y%m'), DATE_FORMAT(IFNULL(MAX(q.complete_day), CURRENT_DATE), '%Y%m')) AS elapsed_month             /* 経過月       */
      FROM q_body           AS q
...
SELECT m_branch_id             AS m_branch_id             , /* 拠点id       */
       m_branch_cd             AS m_branch_cd             , /* 拠点cd       */
       m_branch_name           AS m_branch_name           , /* 拠点名       */
       m_customer_id           AS m_customer_id           , /* 仕入先id     */
       m_customer_cd           AS m_customer_cd           , /* 仕入先cd     */
       m_customer_name         AS m_customer_name         , /* 仕入先名     */
       m_customer_closing_date AS m_customer_closing_date , /* 仕入先締日   */
       m_user_name             AS m_user_name             , /* 担当者名     */
       period                  AS period                  , /* 対象請求日   */
       slip_cnt                AS slip_cnt                , /* 仕入伝票数   */
       accounting_date         AS accounting_date         , /* 最終仕入日   */
       total_price             AS total_price             , /* 仕入額       */
       payment_day             AS payment_day             , /* 最終支払日   */
       payment_price           AS payment_price           , /* 支払額       */
       complete_day            AS complete_day            , /* 最終仕入締日 */
       grand_total             AS grand_total             , /* 仕入締額     */
       elapsed_month           AS elapsed_month             /* 経過月       */
