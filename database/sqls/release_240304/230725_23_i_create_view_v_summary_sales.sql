...
SELECT br.id                                                                             AS m_branch_id           , /* 拠点id     */ 
       ...
       SUM(c.total_price + c.tax - IFNULL(p.payment_price, 0) - IFNULL(p.commission, 0)) AS uncollected           , /* 未収残     */ 
 ...
 GROUP BY m_branch_id   , m_branch_cd   , m_branch_name   ,
          ...
          cu.id         , m_customer_cd , m_customer_name , sales_management_memo , cu.payment_date , 
          grouping_column

 ORDER BY m_branch_cd, m_user_cd, m_customer_cd, due_date
;
