...
use App\Models\SignmisReports\VBalancePurchase;

class VBalancePurchaseApiController extends SignmisReportApiController
{
    public function __construct()
    {
        $this->model = VBalancePurchase::class;
    }
    ...
    public function retrieve(Request $request)
    {
        ...
        $param = [
            ...
            'accounts_title'          => $request['accounts_title'         ] == 0 ? null : $request['accounts_title'         ] ,
        ...
        $details = DB::select(<<<__SQL_EOT__
            ...
            q_purchases AS (
                SELECT h.m_branch_id              AS m_branch_id   , /* 拠点id   */
                       h.supplier_m_customer_id   AS m_customer_id , /* 仕入先id */
                       SUM(h.total_price        ) AS total_price   , /* 仕入     */
                       SUM(                h.tax) AS tax           , /* 消費税   */
                       SUM(h.total_price + h.tax) AS sub_total       /* 仕入合計 */
                  FROM t_p_invoices AS h
                 ...
                 GROUP BY h.m_branch_id, h.supplier_m_customer_id
            ) ,
            ...
            q_tax_summary AS (
                SELECT h.m_branch_id                            AS m_branch_id              , /* 拠点id   */
                  ...
                  FROM t_p_invoices AS h
                  JOIN m_customers  AS cu ON h.supplier_m_customer_id = cu.id
                 WHERE h.deleted_at           IS NULL                                    /* 論理削除         */
                   AND h.t_complete_id        >= 0                                       /* 残高用請求は除外 */
                   AND h.account_m_kv_id      IN(1060003)                                /* 締区分が請求時   */
                   AND h.accounting_date BETWEEN :trading_day_from2 AND :trading_day_to2 /* 仕入日           */
                   AND :tax_calc               = 1
                 GROUP BY h.m_branch_id, h.supplier_m_customer_id, h.t_complete_id, cu.tax_fraction_m_kv_id
            ) ,
            ...
            q_tax AS (
                SELECT m_branch_id                                                 AS m_branch_id   , /* 拠点id   */
                  ...
                  FROM q_tax_summary
                 WHERE tax_flg = 0 /* 指定期間内に請求消費税がない仕入データの消費税が対象 */
            ) ,
            ...
            q_payments AS (
                SELECT p.m_branch_id                                                                  AS m_branch_id        , /* 拠点id       */
                 ...
                 WHERE p.deleted_at           IS NULL                                    /* 論理削除 */
                   ...
                   AND p.payment_day     BETWEEN :trading_day_from3 AND :trading_day_to3 /* 支払日   */
                ...
                SELECT m_branch_id   AS m_branch_id   , /* 拠点id   */
                  ...
                  FROM (/* 初回残高 */
                        ...
                        SELECT p.m_branch_id             AS m_branch_id   , /* 拠点id   */
                         ...
                         WHERE p.deleted_at      IS NULL               /* 論理削除 */
                           AND p.dealing_m_kv_id  = 1010002            /* 買       */
                           AND p.payment_day      < :trading_day_from7 /* 支払日   */
                         GROUP BY p.m_branch_id, p.m_customer_id
                        ...
                        SELECT p.m_branch_id             AS m_branch_id   , /* 拠点id   */
                         ...
                         WHERE p.deleted_at      IS NULL               /* 論理削除 */
                           ...
                           AND p.payment_day      < :trading_day_from8 /* 支払日   */
                         GROUP BY p.m_branch_id, p.m_customer_id
                        ...
                        SELECT h.m_branch_id                                                   AS m_branch_id   , /* 拠点id   */
                         ...
                         WHERE h.deleted_at      IS NULL               /* 論理削除 */
                           AND h.accounting_date  < :trading_day_from9 /* 仕入日   */
                         GROUP BY h.m_branch_id, h.supplier_m_customer_id
                        ...
                        SELECT h.m_branch_id                                                   AS m_branch_id   , /* 拠点id   */
                         ...
                         WHERE h.deleted_at      IS NULL                /* 論理削除 */
                           ...
                           AND h.accounting_date  < :trading_day_from10 /* 仕入日   */
                         GROUP BY h.m_branch_id, h.supplier_m_customer_id
            ...
            q_body AS (
                SELECT m_branch_id                                                      AS m_branch_id        , /* 拠点id       */
                       ...
                       SUM(                               total_price + tax + tax_calc) AS sub_total          , /* 仕入合計     */
            ...
            /* 表示用 */
            SELECT k1.g_01                                AS accounts_title_cd   , /* 勘定科目CD   */
                   k1.kv_name                             AS accounts_title_name , /* 勘定科目名   */
                   ...
                   CAST(q.total_price        AS SIGNED)   AS total_price         , /* 仕入額       */
                   CAST(q.tax                AS SIGNED)   AS tax                 , /* 消費税       */
                   CAST(q.tax_calc           AS SIGNED)   AS tax_calc            , /* 仮消費税     */
                   CAST(q.sub_total          AS SIGNED)   AS sub_total           , /* 仕入合計     */
             ...
             INNER JOIN m_customers AS cu /* 仕入先   */ ON q.m_customer_id           = cu.id
              LEFT JOIN m_kvs       AS k1 /* 勘定科目 */ ON cu.accounts_title_m_kv_id = k1.id
             ...
             WHERE br.id           = IFNULL(:m_branch_id             , br.id          ) /* 拠点id     */
               ...
               AND k1.id           = IFNULL(:accounts_title          , k1.id          ) /* 勘定科目id */

             ORDER BY br.cd, k1.order_no, cu.cd
        __SQL_EOT__, $param);
